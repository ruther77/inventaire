...
from core.price_history_service import (
    fetch_price_history,
...
from backend.services.catalog_data import (
...
PAGE_DEFINITIONS = [
    ("showcase", "Vitrine", "app.py"),
    ("supply", "Approvisionnement", "pages/02_Approvisionnement.py"),
    ("pos", "Vente (PoS)", "pages/03_Vente_PoS.py"),
    ("catalog", "Catalogue", "pages/04_Catalogue.py"),
    ("mvt", "Stock & Mvt", "pages/05_Stock_Mvt.py"),
    ("audit", "Audit & écarts", "pages/06_Audit.py"),
    ("dash", "Dashboard", "pages/07_Dashboard.py"),
    ("scanner", "Scanner", "pages/08_Scanner.py"),
    ("extract", "Extraction Facture", "pages/09_Extraction_Facture.py"),
    ("import", "Importation", "pages/10_Importation.py"),
    ("price_watch", "Suivi Prix", "pages/11_Suivi_Prix.py"),
    ("admin", "Maintenance (Admin)", "pages/12_Maintenance_Admin.py"),
]

PAGE_TITLES = {slug: label for slug, label, _ in PAGE_DEFINITIONS}
PAGE_PATHS = {slug: path for slug, _, path in PAGE_DEFINITIONS}
...
def _hide_native_page_nav() -> None:
    """Masque le panneau de navigation multi-page natif de Streamlit."""
...
def _get_product_select_options(
    *,
    empty_label: str = "-- Sélectionner un produit --",
    error_label: str = "-- Erreur de chargement --",
    show_error: bool = True,
) -> tuple[list[str], dict[str, int]]:
    """Retourne (liste affichage, mapping) pour les sélecteurs produits."""

    try:
        options = cached_product_options()
    except Exception as exc:
        if show_error:
            st.error(f"Impossible de charger la liste des produits: {exc}")
        return [error_label], {}

    names = [empty_label] + sorted(options.keys())
...
def _require_authenticated_user() -> dict[str, Any]:
    """Retourne l'utilisateur connecté ou redirige vers la page d'accueil."""
...
def _redirect_to_login() -> None:
    """Redirige vers la page d'accueil en gérant les versions Streamlit plus anciennes."""
...
def run_app(active_page: str | None = None):
    current_page = active_page or "showcase"
    if current_page not in PAGE_TITLES:
        st.set_page_config(page_title="Inventaire Épicerie", layout="wide", page_icon="📦")
        st.error(f"Page Streamlit inconnue: {current_page}")
        st.stop()
    page_label = PAGE_TITLES[current_page]

    # --- CONFIGURATION DE LA PAGE ---
    st.set_page_config(
        page_title=f"{page_label} · Inventaire Épicerie",
        layout="wide",
        page_icon="📦",
    )
...
if "invoice_reception_summary" not in st.session_state:
    st.session_state["invoice_reception_summary"] = None
    ...
    # Assure ``invoice_text_input`` remains the source of truth. If one of the
    # tab-specific widgets is out-of-sync, propagate the canonical value to it
    # rather than overwriting the central state with an outdated copy.
    if _extract_text_state != _central_invoice_text:
        st.session_state["extract_invoice_text_input"] = _central_invoice_text
    if _import_text_state != _central_invoice_text:
        st.session_state["import_invoice_text_input"] = _central_invoice_text
    ...
    def _schedule_invoice_text_update(new_text: str) -> None:
        st.session_state["_pending_invoice_text"] = new_text
    ...
    def render_global_product_detail_popover() -> None:
        """Affiche une bulle de fiche produit flottante persistante sur toute l'app."""
        ...
        with detail_popover:
            ...
            current_selection = st.session_state.get("catalog_detail_select")
            if current_selection and current_selection not in options:
                st.session_state.pop("catalog_detail_select", None)
            ...
            price_history = fetch_price_history_for_product(
                produit_id=int(detail_row["id"]),
                code=ean_value or None,
                limit=120,
            )
            st.markdown("#### Historique des prix d'achat")
            if price_history.empty:
                st.caption("Aucun historique de prix enregistré pour ce produit pour l'instant.")
            else:
                price_history["facture_date"] = (
                    pd.to_datetime(price_history["facture_date"], errors="coerce", utc=True)
                    .dt.tz_convert(None)
                )
                price_history = price_history.sort_values("facture_date")
                latest_price = float(price_history["prix_achat"].iloc[-1])
                reference_price = float(price_history["prix_achat"].iloc[0])
                reference_date = price_history["facture_date"].iloc[0]
                delta_value = latest_price - reference_price
                st.metric(
                    "Dernier prix",
                    f"{latest_price:.2f} €",
                    delta=f"{delta_value:+.2f} € vs {reference_date.strftime('%d/%m/%Y')}",
                )

                chart_slice = price_history.tail(20)
                fig = px.line(
                    chart_slice,
                    x="facture_date",
                    y="prix_achat",
                    markers=True,
                    hover_data=["fournisseur", "source_context"],
                )
                fig.update_layout(
                    height=260,
                    margin=dict(l=10, r=10, t=40, b=10),
                    yaxis_title="Prix (€)",
                    xaxis_title="Date facture",
                )
                st.plotly_chart(fig, use_container_width=True, key="product_price_history_chart")
    ...
    def _set_active_invoice_from_index(index: int) -> None:
        ...
        _schedule_invoice_text_update(batch["text"])
        for text_key in ("extract_invoice_text_input", "import_invoice_text_input"):
            st.session_state[text_key] = batch["text"]
        st.session_state["invoice_products_df"] = None
        st.session_state["invoice_import_summary"] = None
    ...
    def _prepare_invoice_dataframe(source_df: pd.DataFrame, margin_rate: float) -> pd.DataFrame:
        """Wrapper autour du helper partagé pour conserver la compatibilité Streamlit."""

        return prepare_invoice_dataframe(source_df, margin_rate)
    ...
    st.session_state["user_role"] = current_user.get("role", "standard")
    st.session_state["username"] = current_user.get("username", "")

    st.title("📦 Inventaire — Gestion Complète")
    st.sidebar.caption(
        f'Bienvenue, **{current_user.get("username", "Utilisateur")}** '
        f'(Rôle: **{st.session_state["user_role"]}**)'
    )
    st.sidebar.markdown("### Navigation")
    for slug, label, path in PAGE_DEFINITIONS:
    ...
    render_global_product_detail_popover()
    ...
    if current_page == "pos":
        ...
        with input_col:
            with workspace_panel(
                ...
                pos_product_names, product_options = _get_product_select_options()
    ...
    # ---------------- Catalogue ----------------
        ...
        if st.session_state.get("user_role") == "admin":
            with workspace_panel(
                ...
                adjustment_product_names, adjustment_product_options = _get_product_select_options(
                    empty_label="-- Sélectionner un produit --",
                    error_label="-- Aucun produit disponible --",
                    show_error=True,
                )

                with st.form("stock_adjustment_form", clear_on_submit=True):
                    col_prod, col_qty = st.columns(2)
                    selected_product_name = col_prod.selectbox(
                        "Produit à ajuster",
                        adjustment_product_names,
                        key="adj_product",
                    )
                    selected_product_id = adjustment_product_options.get(selected_product_name)
                    product_details = (
    ...
    if current_page == "extract":
        ...
        if isinstance(extracted_df, pd.DataFrame) and not extracted_df.empty:
            with workspace_panel(
                ...
                margin_default = float(st.session_state.get("invoice_margin_percent", 40.0))
                ...
                invoice_selected_date = st.session_state.get("invoice_selected_date", date.today())
                invoice_date_input = st.date_input(
                    "Date de la facture",
                    value=invoice_selected_date,
                    key="invoice_date_input_extract",
                )
                st.session_state["invoice_selected_date"] = invoice_date_input
                ...
                with col_import:
                    if st.button("Importer ces produits", key="extract_invoice_import_button", type="primary"):
                        ...
                        selected_invoice_date = st.session_state.get("invoice_selected_date")
                        if isinstance(selected_invoice_date, datetime):
                            invoice_dt = selected_invoice_date if selected_invoice_date.tzinfo else selected_invoice_date.replace(tzinfo=timezone.utc)
                        elif isinstance(selected_invoice_date, date):
                            invoice_dt = datetime.combine(selected_invoice_date, time.min, tzinfo=timezone.utc)
                        else:
                            invoice_dt = datetime.now(timezone.utc)

                        record_price_history(
                            editable_df,
                            supplier=st.session_state.get("invoice_supplier_hint"),
                            context="Extraction",
            ...
            with workspace_panel(
                ...
                if not isinstance(reconciliation_df, pd.DataFrame) or reconciliation_df.empty:
                    ...
                    margin_percent = float(st.session_state.get("invoice_margin_percent", 40.0))
                    margin_rate = margin_percent / 100.0
                    ...
                    metrics_cols = st.columns(4)
                    identified_products = (
                        int(working_df["produit_id"].notna().sum())
                        if "produit_id" in working_df.columns
                        else 0
                    )
                    metrics_cols[0].metric("Lignes rapprochées", f"{len(working_df):,}".replace(",", " "))
                    metrics_cols[1].metric(
                        "Produits identifiés",
                        f"{identified_products:,}".replace(",", " "),
                    )
                    ...
                    supplier_name = form_cols[0].text_input(
                        "Fournisseur / source",
                        value=st.session_state.get("invoice_supplier_hint", ""),
                        key="invoice_supplier_input_extract",
                    )
                    if supplier_name:
                        st.session_state["invoice_supplier_hint"] = supplier_name.strip()
                    ...
                    if "produit_id" in working_df.columns:
                        ...
                        movements_df = working_df.iloc[0:0].copy()
                    ...
                    with action_cols[0]:
                        if st.button(
                            ...
                            if action_mode != "Créer mouvements d'entrée":
                                ...
                                st.session_state["invoice_reception_summary"] = result
    ...
    if current_page == "import":
        ...
        reception_summary = st.session_state.get("invoice_reception_summary")
        pending_count = int(len(imported_df)) if isinstance(imported_df, pd.DataFrame) else 0

        render_workspace_hero(
            eyebrow="Importation",
            title="Injectez les quantités réellement réceptionnées",
            description="Utilisez les lignes extraites pour créer des mouvements d'entrée. Les mises à jour catalogue se font dans l'onglet « Extraction facture ».",
            badges=["Appro logistique", "Mouvements sécurisés"],
            metrics=[
                {"label": "Lignes prêtes", "value": str(pending_count)},
                {
                    "label": "Dernière réception",
                    "value": (
                        f"{reception_summary.get('movements_created', 0)} mvts"
                        if isinstance(reception_summary, dict)
                        else "À créer"
                    ),
                },
            ],
        ...
        if isinstance(imported_df, pd.DataFrame) and not imported_df.empty:
            with workspace_panel(
                ...
                col_download, col_notice = st.columns(2)
                with col_download:
                    csv_data = editable_df.to_csv(index=False).encode("utf-8")
                    st.download_button(
                        "Télécharger le CSV extrait",
                ...
                with col_notice:
                    st.info(
                        "Les mises à jour du catalogue et des prix se font désormais depuis l'onglet "
                        "**Extraction facture**. Cette page se concentre uniquement sur les mouvements de stock.",
                        icon="ℹ️",
                    )
        ...
        if isinstance(reception_summary, dict):
            with workspace_panel(
                ...
                "Synthèse des mouvements générés à partir des factures rapprochées.",
                icon="✅",
                accent="teal",
            ):
                metric_cols = st.columns(3)
                metric_cols[0].metric(
                    "Mouvements créés", reception_summary.get("movements_created", 0)
                )
                metric_cols[1].metric(
                    "Quantité totale", f"{reception_summary.get('quantity_total', 0):.2f} u"
                )
                metric_cols[2].metric(
                    "Dernier fournisseur",
                    reception_summary.get("supplier") or st.session_state.get("invoice_supplier_hint") or "—",
                )

                if reception_summary.get("details"):
                    detail_df = pd.DataFrame(reception_summary["details"])
                    st.dataframe(detail_df, hide_index=True, use_container_width=True)
                else:
                    st.info("Aucun détail supplémentaire n'a été enregistré pour cette réception.")

    if current_page == "price_watch":
        st.header("Suivi de l'évolution des prix d'achat")

        default_end = datetime.now()
        default_start = default_end - timedelta(days=180)

        search_col, supplier_col, date_col, limit_col = st.columns([1.4, 1.0, 1.2, 0.8])
        search_filter = search_col.text_input(
            "Produit / Code-barres",
            key="price_watch_search",
            placeholder="Ex: Vodka, 5010327…",
        )
        supplier_filter = supplier_col.text_input(
            "Fournisseur",
            key="price_watch_supplier",
            placeholder="Nom fournisseur",
        )
        date_range_value = date_col.date_input(
            "Période observée",
            value=(
                st.session_state.get("price_watch_start", default_start.date()),
                st.session_state.get("price_watch_end", default_end.date()),
            ),
            key="price_watch_date_range",
        )
        limit_value = int(
        ...
        st.session_state["price_watch_start"] = start_date
        st.session_state["price_watch_end"] = end_date
        ...
        total_points = int(len(price_history_df))
        total_products = int(price_history_df["code"].nunique()) if not price_history_df.empty else 0
        avg_price = (
            float(price_history_df["prix_achat"].mean()) if not price_history_df.empty else 0.0
        )

        render_workspace_hero(
            eyebrow="Inflation & Promotions",
            title="Visualisez l'évolution des coûts fournisseurs",
            description="Analysez les hausses, repérez les promotions et identifiez vos produits sensibles.",
            badges=["Historique factures", "Alertes tarifaires"],
            metrics=[
                {"label": "Points historisés", "value": f"{total_points:,}".replace(",", " ")},
                {"label": "Produits suivis", "value": f"{total_products:,}".replace(",", " ")},
                {"label": "Prix moyen", "value": f"{avg_price:.2f} €"},
            ],
            tone="marine",
        )

        if price_history_df.empty:
            st.info("Aucun historique disponible pour les filtres indiqués.")
        else:
            price_history_df["facture_date"] = (
                pd.to_datetime(price_history_df["facture_date"], errors="coerce", utc=True)
                .dt.tz_convert(None)
            )
            price_history_df["code_label"] = (
                price_history_df["code"].fillna(price_history_df["nom"]).fillna("—")
            )

            ordered_history = price_history_df.sort_values("facture_date")
            summary_df = (
                ordered_history.groupby("code_label", dropna=False)
                .agg(
                    produit_nom=("nom", "last"),
                    fournisseur=("fournisseur", "last"),
                    last_price=("prix_achat", "last"),
                    first_price=("prix_achat", "first"),
                    max_price=("prix_achat", "max"),
                    min_price=("prix_achat", "min"),
                    last_date=("facture_date", "last"),
                    points=("prix_achat", "count"),
                )
                .reset_index()
            )
            summary_df["variation_abs"] = summary_df["last_price"] - summary_df["first_price"]
            summary_df["variation_pct"] = summary_df.apply(
                lambda row: (
                    ((row["last_price"] - row["first_price"]) / row["first_price"]) * 100
                    if row["first_price"] not in (0, None)
                    else 0
                ),
                axis=1,
            )

            avg_variation = float(summary_df["variation_abs"].mean()) if not summary_df.empty else 0.0
            avg_amplitude = float((summary_df["max_price"] - summary_df["min_price"]).mean()) if not summary_df.empty else 0.0
            avg_points = float(summary_df["points"].mean()) if not summary_df.empty else 0.0
            ...
            top_codes = (
                summary_df.sort_values("variation_abs", key=lambda s: s.abs(), ascending=False)
                .head(6)["code_label"]
                .tolist()
            )

            chart_df = ordered_history[ordered_history["code_label"].isin(top_codes)]
            ...
            st.dataframe(
                ...
                use_container_width=True,
                column_config={
                    "code_label": "Produit / Code",
                    "produit_nom": "Nom catalogue",
                    "fournisseur": "Fournisseur",
                    "last_price": st.column_config.NumberColumn("Dernier prix (€)", format="%.2f €"),
                    "last_date": st.column_config.DatetimeColumn("Dernière facture"),
                    "variation_abs": st.column_config.NumberColumn("Variation (€)", format="%+.2f €"),
                    "variation_pct": st.column_config.NumberColumn("Variation (%)", format="%+.2f %%"),
                    "max_price": st.column_config.NumberColumn("Prix max (€)", format="%.2f €"),
                    "min_price": st.column_config.NumberColumn("Prix min (€)", format="%.2f €"),
                    "points": st.column_config.NumberColumn("Points"),
                },
            )

            export_csv = summary_df.to_csv(index=False).encode("utf-8")
            st.download_button(
                "Exporter la synthèse (CSV)",
                data=export_csv,
                file_name="historique_prix.csv",
                mime="text/csv",
                key="price_watch_export_csv",
            )
    ...
    if current_page == "admin":
        if st.session_state["user_role"] != "admin":
            ...
            with workspace_panel(
                "Comptes & rôles",
                "Créez, listez et administrez les comptes Streamlit.",
                icon="👥",
                accent="emerald",
            ):
                users_df = list_users()
                if users_df.empty:
                    st.info("Aucun compte utilisateur pour l'instant. Utilisez la page d'accueil pour en créer.")
                else:
                    display_df = users_df.copy()
                    display_df["created_at"] = (
                        pd.to_datetime(display_df["created_at"]).dt.strftime("%d/%m/%Y %H:%M")
                    )
                    st.dataframe(
                        display_df.rename(
                            columns={
                    ...
                    st.divider()
                    st.subheader("Mettre à jour un rôle")
                    with st.form("admin_user_role_form"):
                        target_user_id = st.selectbox(
                            "Utilisateur",
                            options=user_ids,
                            format_func=lambda uid: user_labels.get(uid, f"#{uid}"),
                        )
                        new_role = st.selectbox(
                            "Rôle",
                            options=list(ALLOWED_ROLES),
                            format_func=lambda role: role_display.get(role, role),
                        )
                        role_submit = st.form_submit_button("Enregistrer le rôle")
                    ...
                    if role_submit:
                        try:
                            ...
                            st.success("Rôle mis à jour avec succès.")
                            try:
                                st.rerun()
                            except AttributeError:
                                st.experimental_rerun()
                    ...
                    with st.form("admin_user_reset_pwd_form"):
                        reset_user_id = st.selectbox(
                            "Utilisateur",
                            options=user_ids,
                            format_func=lambda uid: user_labels.get(uid, f"#{uid}"),
                            key="reset_pwd_user_select",
                        )
                        new_password = st.text_input(
                            "Nouveau mot de passe (laisser vide pour une génération sécurisée)",
                            type="password",
                        )
                    ...
                    if reset_submit:
                        try:
                            fresh_password = reset_user_password(reset_user_id, new_password or None)
                        except ValueError as exc:
                            st.error(str(exc))
                        else:
                            st.success(
                                "Mot de passe réinitialisé. Communiquez-le via un canal sécurisé puis demandez "
                                "à l'utilisateur de le modifier dès la prochaine connexion."
                            )
                            st.code(fresh_password, language="text")
