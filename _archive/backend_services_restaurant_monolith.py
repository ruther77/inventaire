...
def _get_restaurant_entity_id() -> int:
    """Retourne l'entity_id finance associée au restaurant (code RESTO)."""
    ...
    raise RuntimeError("Aucune entité finance n'a été trouvée (code RESTO manquant ?)")
...
def list_bank_statements(tenant_id: int, account: str | None = None) -> List[dict[str, Any]]:
    """Retourne les lignes de relevés bancaires en s'appuyant sur finance_* (filtrage par compte possible)."""
    ...
    sql = text(
        ...
        SELECT
          ...
          CASE WHEN t.direction = 'IN' THEN 'Entrée' ELSE 'Sortie' END AS type,
