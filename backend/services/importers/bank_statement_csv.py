...
def parse_csv(content: bytes | str) -> list[dict[str, Any]]:
    ...
    for raw in reader:
        libelle_raw = (raw.get("libelle") or raw.get("label") or "").strip()
        ref_raw = (raw.get("ref") or raw.get("reference") or "").strip() or None
        libelle = libelle_raw or ref_raw or "Inconnu"
...
def _checksum(account_id: int, entry: dict[str, Any]) -> str:
    ref = (entry.get("ref_banque") or "").strip().lower()
    base = f"{account_id}|{entry['date_operation']}|{abs(entry['montant'])}|{ref}"
