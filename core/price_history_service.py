...
# Désactive le downcasting silencieux pour éviter FutureWarning
...
def _normalize_code(value) -> str | None:
    ...
    # Rejette les valeurs "nan" qui viennent de str(float('nan'))
...
def fill_missing_prices_from_history(tenant_id: int = 1, margin_multiplier: float | None = None) -> dict:
    ...
    if not _TABLE_READY:
        return {"updated": 0}
    multiplier = margin_multiplier or _MARGIN_MULTIPLIER
    sql = text(
        """
        WITH latest AS (
            SELECT ph.code,
                   ...
                   pb.produit_id
            FROM produits_price_history ph
            JOIN produits_barcodes pb ON pb.code = ph.code AND pb.tenant_id = :tenant_id
            WHERE ph.tenant_id = :tenant_id
            QUALIFY ROW_NUMBER() OVER (PARTITION BY ph.code ORDER BY ph.facture_date DESC, ph.created_at DESC) = 1
        ),
        candidates AS (
            SELECT
                l.produit_id,
                ...
                p.tva
            FROM latest l
            JOIN produits p ON p.id = l.produit_id AND p.tenant_id = :tenant_id
            WHERE (p.prix_achat IS NULL OR p.prix_achat <= 0)
               OR (p.prix_vente IS NULL OR p.prix_vente <= 0)
        )
        UPDATE produits p
        SET
            prix_achat = COALESCE(p.prix_achat, c.prix_achat),
            prix_vente = COALESCE(
                p.prix_vente,
                ROUND(c.prix_achat * :multiplier * (1 + COALESCE(c.tva,0) / 100), 4)
            )
        FROM candidates c
        WHERE p.id = c.produit_id AND p.tenant_id = :tenant_id
        RETURNING p.id
        """
    )
    try:
        rows = exec_sql(sql, {"tenant_id": int(tenant_id), "multiplier": multiplier})
        return {"updated": len(rows)}
    except Exception as exc:
        LOGGER.warning("Impossible de remplir les prix manquants depuis l'historique: %s", exc)
        return {"updated": 0}
...
def record_price_history(
    ...
    *,
    ...
    nom_column = _get_column(column_index, ["nom", "name", "designation", "libelle"])  # Colonne nom produit
    ...
    for record in df.to_dict(orient="records"):  # Itère sur chaque ligne de la facture
        ...
        # Fallback sur le nom du produit si pas de code EAN
        if not code_value:
            nom_value = record.get(nom_column) if nom_column else record.get("nom")
            if nom_value and isinstance(nom_value, str) and nom_value.strip():
                # Génère un code placeholder basé sur le hash du nom (cohérent avec products_loader)
                import hashlib
                nom_hash = hashlib.md5(nom_value.strip().upper().encode()).hexdigest()[:10]
                code_value = f"PLH{nom_hash.upper()}"

        if not code_value:  # Code toujours manquant
...
def fetch_price_history(
    *,
    ...
    for col in bool_cols:
        if col in df.columns:
            # Évite FutureWarning: conversion explicite sans downcasting
            df[col] = df[col].astype(object).fillna(False).astype(bool)

    if "stockout_events" in df.columns:
        # Évite FutureWarning: conversion explicite
        df["stockout_events"] = pd.to_numeric(df["stockout_events"], errors="coerce").astype("Int64").fillna(0).astype(int)
