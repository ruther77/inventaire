...
import { fetchCockpitOverview, fetchCockpitActions } from '../../api/newcms.js';
...
export default function CockpitPage() {
  ...
  const overviewQuery = useNewCMSQuery(['newcms-cockpit-overview'], () => fetchCockpitOverview());
  const actionsQuery = useNewCMSQuery(['newcms-cockpit-actions'], () => fetchCockpitActions());

  const kpis = useMemo(() => {
    const k = overviewQuery.data?.kpis;
    if (!k) return null;
    return [
      { label: 'Valeur stock', value: k.stock_value?.value, trend: k.stock_value?.trend_direction, trendValue: `${k.stock_value?.trend ?? ''}%`, icon: Package },
      { label: 'Alertes stock', value: k.stock_alerts?.value, trend: k.stock_alerts?.trend_direction, trendValue: `${k.stock_alerts?.trend ?? ''}%`, icon: AlertTriangle },
      { label: 'Marge moyenne', value: `${k.margin_avg?.value ?? ''}${k.margin_avg?.unit ?? ''}`, trend: k.margin_avg?.trend_direction, trendValue: `${k.margin_avg?.trend ?? ''}%`, icon: Receipt },
      { label: 'Cash flow 7j', value: k.cash_flow_7d?.value, trend: k.cash_flow_7d?.trend_direction, trendValue: `${k.cash_flow_7d?.trend ?? ''}%`, icon: DollarSign },
      { label: 'Anomalies', value: k.anomalies_count?.value, trend: k.anomalies_count?.trend_direction, trendValue: `${k.anomalies_count?.trend ?? ''}%`, icon: AlertTriangle },
      { label: 'Taux match', value: `${k.match_rate?.value ?? ''}${k.match_rate?.unit ?? ''}`, trend: k.match_rate?.trend_direction, trendValue: `${k.match_rate?.trend ?? ''}%`, icon: BarChart3 },
    ].filter((item) => item.value !== undefined && item.value !== null);
  }, [overviewQuery.data]);

  const actions = actionsQuery.data?.actions || overviewQuery.data?.actions || [];
  const alerts = overviewQuery.data?.alerts || [];
  const aiSuggestions = overviewQuery.data?.ai_suggestions || [];
  const isLoading = overviewQuery.isLoading || actionsQuery.isLoading;
  const isError = overviewQuery.isError || actionsQuery.isError;

  return (
    <div className="min-h-screen p-6 space-y-6">
      {isError && (
        <div className="rounded-xl border border-rose-500/40 bg-rose-500/10 px-4 py-3 text-sm text-rose-200">
          Impossible de charger les données Cockpit (API newCMS). Affichage du fallback démo.
        </div>
      )}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          {(kpis && kpis.length
            ? kpis.map((kpi) => ({
                ...
                trendValue: kpi.trend_value || kpi.delta || '—',
                icon: kpi.icon || DollarSign,
              }))
            : [
                { label: 'CA Jour', value: '2 847 €', trend: 'up', trendValue: '+12%', icon: DollarSign },
                { label: 'Marge Brute', value: '38.2%', trend: 'down', trendValue: '-2.1%', icon: Receipt },
                { label: 'Tickets', value: '127', trend: 'up', trendValue: '+8', icon: ShoppingCart },
                { label: 'Panier Moyen', value: '22.4 €', trend: 'flat', trendValue: '→ stable', icon: Package },
              ]).map((kpi, idx) => (
            <KPICard
              key={idx}
              label={kpi.label}
      ...
      <div className="p-6 rounded-2xl bg-gradient-to-br from-slate-800/50 to-slate-900/50 border border-white/10">
        ...
        <div className="space-y-3">
          {(actions.length ? actions : [
            ...
            <ActionCard
              key={action.id || idx}
              urgency={action.urgency || action.category || 'info'}
              title={action.title || action.label}
              description={action.description}
              suggestion={action.explanation || action.suggestion}
              actions={action.actions || [
                { label: 'Voir', primary: true },
                { label: 'Plus tard' },
              ]}
            />
          ))}
